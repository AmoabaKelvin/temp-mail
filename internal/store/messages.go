...
func (s *MessageStore) Get(ctx context.Context, id int64) ([]Message, error) {
	ctx, cancel := context.WithTimeout(ctx, QueryDurationTimeout)
	defer cancel()

	query := `SELECT id, from_address, to_address_id, headers, subject, body, received_at, read_at FROM messages WHERE id = $1`
	messages := []Message{}
	...
	for rows.Next() {
		var message Message
		err := rows.Scan(&message.ID, &message.FromAddress, &message.ToAddressID, &message.Headers, &message.Subject, &message.Body, &message.ReceivedAt, &message.ReadAt)
		if err != nil {
			return nil, err
		}
...
func (s *MessageStore) Create(ctx context.Context, message *Message) error {
	...
	query := `INSERT INTO messages (from_address, to_address_id, subject, body, headers, received_at) 
